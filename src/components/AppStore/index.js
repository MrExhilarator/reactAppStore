...
class AppStore extends Component {
  ...
  render() {
    const {searchString, tab} = this.state
    const categorizedApps = appsList.filter(app => app.category === tab)
    const filteredApps = categorizedApps.filter(app =>
      app.appName.toLowerCase().includes(searchString.toLowerCase()),
    )
        ...
        <ul className="tabs-list">
          {tabsList.map(tabItem => (
            <TabItem
              ...
              isSelected={tab === tabItem.tabId}
            />
          ))}
        </ul>
        <ul className="apps-list">
          {filteredApps.map(app => (
            <AppItem key={app.appId} appInfo={app} />
          ))}
        </ul>
      </div>
    )
